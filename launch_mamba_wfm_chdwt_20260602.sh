...
PY_CMD=("$CONDA" run --no-capture-output -n wfanet_mamba python -u train_rssm_hz.py)
...
echo "started gf2_mamba_wfm_chdwt_l1_80ep_from120 pid=$!"

nohup "${PY_CMD[@]}" --gpu 1 "${COMMON[@]}" \
...
echo "started gf2_mamba_wfm_chdwt_mseband_80ep_from120 pid=$!"

nohup "${PY_CMD[@]}" --gpu 2 "${COMMON[@]}" \
...
echo "started qb_mamba_wfm_chdwt_mseband_80ep_from120 pid=$!"

nohup "${PY_CMD[@]}" --gpu 3 "${COMMON[@]}" \
...
echo "started qb_mamba_wfm_chdwt_samll_80ep_from120 pid=$!"
