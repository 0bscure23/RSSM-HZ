...
"${PY_CMD[@]}" --gpu 0 "${COMMON[@]}" \
  ...
  --w-sam 0 \
  --w-ll 0 \
  --run-tag gf2_state_from_reduce_l1_35ep \
  > output_log/gf2_state_from_reduce_l1_20260601.log 2>&1 &
echo "started gf2_state_from_reduce_l1_35ep pid=$!"
...
"${PY_CMD[@]}" --gpu 3 "${COMMON[@]}" \
  ...
  --w-sam 0.01 \
  --w-ll 0.02 \
  --run-tag qb_state_from_reduce_samll_35ep \
  > output_log/qb_state_from_reduce_samll_20260601.log 2>&1 &
echo "started qb_state_from_reduce_samll_35ep pid=$!"
