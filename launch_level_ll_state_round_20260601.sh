...
COMMON=(
  ...
  --w-wavelet-hf 0
  --w-kl 0
  --phase-b-freeze-mode state_gate_head
...
"${PY_CMD[@]}" --gpu 0 "${COMMON[@]}" "${GF2[@]}" \
  --w-sam 0 \
  --w-ll 0 \
  --run-tag gf2_level_ll_state_l1_50ep \
  > output_log/gf2_level_ll_state_l1_20260601.log 2>&1 &
echo "started gf2_level_ll_state_l1_50ep pid=$!"

"${PY_CMD[@]}" --gpu 1 "${COMMON[@]}" "${QB[@]}" \
  --w-sam 0 \
  --w-ll 0 \
  --run-tag qb_level_ll_state_l1_50ep \
  > output_log/qb_level_ll_state_l1_20260601.log 2>&1 &
echo "started qb_level_ll_state_l1_50ep pid=$!"

"${PY_CMD[@]}" --gpu 2 "${COMMON[@]}" "${GF2[@]}" \
  --w-sam 0.01 \
  --w-ll 0.02 \
  --run-tag gf2_level_ll_state_samll_50ep \
  > output_log/gf2_level_ll_state_samll_20260601.log 2>&1 &
echo "started gf2_level_ll_state_samll_50ep pid=$!"

"${PY_CMD[@]}" --gpu 3 "${COMMON[@]}" "${QB[@]}" \
  --w-sam 0.01 \
  --w-ll 0.02 \
  --run-tag qb_level_ll_state_samll_50ep \
  > output_log/qb_level_ll_state_samll_20260601.log 2>&1 &
echo "started qb_level_ll_state_samll_50ep pid=$!"
