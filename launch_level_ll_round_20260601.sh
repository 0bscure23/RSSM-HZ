...
"${PY_CMD[@]}" --gpu 0 "${COMMON[@]}" "${GF2[@]}" \
  --w-sam 0 \
  --w-ll 0 \
  --run-tag gf2_level_ll_reduce_l1_60ep \
  > output_log/gf2_level_ll_reduce_l1_20260601.log 2>&1 &
echo "started gf2_level_ll_reduce_l1_60ep pid=$!"

"${PY_CMD[@]}" --gpu 1 "${COMMON[@]}" "${QB[@]}" \
  --w-sam 0 \
  --w-ll 0 \
  --run-tag qb_level_ll_reduce_l1_60ep \
  > output_log/qb_level_ll_reduce_l1_20260601.log 2>&1 &
echo "started qb_level_ll_reduce_l1_60ep pid=$!"

"${PY_CMD[@]}" --gpu 2 "${COMMON[@]}" "${GF2[@]}" \
  --w-sam 0.02 \
  --w-ll 0.03 \
  --run-tag gf2_level_ll_reduce_samll_60ep \
  > output_log/gf2_level_ll_reduce_samll_20260601.log 2>&1 &
echo "started gf2_level_ll_reduce_samll_60ep pid=$!"

"${PY_CMD[@]}" --gpu 3 "${COMMON[@]}" "${QB[@]}" \
  --w-sam 0.02 \
  --w-ll 0.03 \
  --run-tag qb_level_ll_reduce_samll_60ep \
  > output_log/qb_level_ll_reduce_samll_20260601.log 2>&1 &
echo "started qb_level_ll_reduce_samll_60ep pid=$!"
