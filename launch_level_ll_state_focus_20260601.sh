...
"${PY_CMD[@]}" --gpu 2 "${COMMON[@]}" \
  ...
  --w-sam 0 \
  --w-ll 0 \
  --run-tag gf2_level_ll_state_l1_focus_40ep \
  > output_log/gf2_level_ll_state_l1_focus_20260601.log 2>&1 &
echo "started gf2_level_ll_state_l1_focus_40ep pid=$!"
...
"${PY_CMD[@]}" --gpu 1 "${COMMON[@]}" \
  ...
  --w-sam 0.01 \
  --w-ll 0.02 \
  --run-tag qb_level_ll_state_samll_focus_40ep \
  > output_log/qb_level_ll_state_samll_focus_20260601.log 2>&1 &
echo "started qb_level_ll_state_samll_focus_40ep pid=$!"
