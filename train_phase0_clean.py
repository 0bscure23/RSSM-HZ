"""Clean Phase-0 training for WFANet/RSSM-HZ architecture comparisons.

This script intentionally avoids warm-start, distillation, dataset-specific
loss stacking, and phase-B finetuning. It is meant to answer one question:
under the same training budget, how do the main architecture families compare?
"""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=[
            "wfanet",
            "rssm_orig3_plain",
            "rssm_two_stage_plain",
            "rssm_two_stage_dw_window",
            "rssm_phase1_shared_dw_window_noll",
            "rssm_phase1_shared_dw_window",
            "rssm_phase1_shared_dw_window_lfm",
        ],
        required=True,
    )
...
def build_model(kind, c_ms, c_pan, hidden_dim, latent_dim):
    ...
    use_two_stage = (
        kind.startswith("rssm_two_stage")
        or kind.startswith("rssm_phase1")
    )
    use_shared = kind.startswith("rssm_phase1")
    no_ll_kinds = {"rssm_phase1_shared_dw_window_noll"}
    ...
    model = RSSMHWViTHZ(
        ...
        freq_state_mode="simple" if use_two_stage else "mixed",
        use_wfanet_two_stage=use_two_stage,
        share_scale_recurrent=use_shared,
        use_level_ll_corr=use_shared and kind not in no_ll_kinds,
        use_local_freq_mixer=(kind == "rssm_phase1_shared_dw_window_lfm"),
    )

    # Match train_rssm_hz.py fresh-start behavior: start from LMS residual path.
...
def forward_model(model, kind, pan, ms, lms):
...
def tiled_forward(model, kind, pan, ms, lms, tile_size=64, pad=8):
    ...
    for yi in range(n_h):
        for xi in range(n_w):
            ...
            pred_tile = forward_model(model, kind, pan_tile, ms_tile, lms_tile).clamp(0.0, 1.0)
...
def eval_model(model, kind, data, device, ratio, q_win_size, batch_size):
    model.eval()
    outs = []
    with torch.no_grad():
        n = data["pan"].shape[0]
...
def main():
    ...
    run_tag = args.run_tag or (
        f"phase0clean_{args.dataset}_{args.model_kind}_"
        f"s{args.seed}_ep{args.epochs}"
    )
    out_dir = os.path.join(ROOT, "results_rssm_hz", run_tag)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for start in range(0, n_train - args.batch_size + 1, args.batch_size):
            ...
            pred = forward_model(model, args.model_kind, pan, ms, lms)
    ...
    with open(os.path.join(eval_dir, "phase0_metrics.json"), "w") as f:
